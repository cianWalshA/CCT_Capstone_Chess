...
stockfish_options = {'Clear Hash':True, 'Threads': 16}
...
#    outTurn = []
    ...
    for move in gameMoves.mainline_moves():
        ...
        if moveCount<games['halfMoveCount'] :
            pass
        elif ((moveCount-games['halfMoveCount'])/10)==4:
            break
...
#            outMove.append(move)
#            outTurn.append(board.turn)
    linesProcessed += 1
    if linesProcessed%1000 == 0:
        print(linesProcessed)
        print((time.time() - start_time))
    return evalList1, depthList1 #,outMove, outTurn
...
#lichessDataSummary2 = lichessData['Opening'].value_counts()
